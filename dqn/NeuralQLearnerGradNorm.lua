...
local nql = torch.class('dqn.NeuralQLearnerGradNorm')
...
function nql:__init(args)
    ...
    self.r_pos_min = 10000
    self.start_norm = 0.7
    -- parameters for reward penalty 
    self.no_rew_duration = 0
    self.no_rew_max = 100
...
function nql:getQUpdate(args)
    local s, a, r, s2, term, delta, delta2
    local q, q2, q2_max, q2_max2, q2_2
    ...
    q2_max2 = self.network:forward(s2):float():max(2)

    -- Compute q2 = (1-terminal) * gamma * max_a Q(s2, a)
    q2 = q2_max:clone():mul(self.discount):cmul(term)
    q2_2 = q2_max2:clone():mul(self.discount):cmul(term)

    delta = r:clone():float()
    delta2 = q2:clone():float():add(-1,q2_2)

    --print("r before", delta)
    ...
    --local tmp = delta:clone():float()

    if self.clip_delta then
        delta[delta:ge(self.clip_delta)] = self.clip_delta
        delta[delta:le(-self.clip_delta)] = -self.clip_delta
    end
    --print(torch.cat(tmp, delta,2))

    local targets1 = torch.zeros(self.minibatch_size, self.n_actions):float()
    local targets2 = torch.zeros(self.minibatch_size, self.n_actions):float()
    for i=1,math.min(self.minibatch_size,a:size(1)) do
        targets1[i][a[i]] = delta[i]
        targets2[i][a[i]] = delta2[i]
    end

    local tmp = torch.cat(targets1[1], targets2[1],2)  
    targets1:mul(0.7)
    targets2:mul(0.3)   
    ...
    return targets1, targets2, delta, q2_max
end
...
function nql:qLearnMinibatch()
    ...
    local targets, targets2, delta, q2_max, delta2 = self:getQUpdate{s=s, a=a, r=r, s2=s2,
    ...
    --print(self.lr_start, self.lr_end, self.lr)
   ...
   -- do update with targets2
    self.network:backward(s, targets2)
...
function nql:perceive(reward, rawstate, terminal, testing, testing_ep)
    ...
    if rerminal or reward>0 then
        self.no_rew_duration = 0
    else
        self.no_rew_duration = self.no_rew_duration + 1
    end
	
    if self.no_rew_duration > self.no_rew_max then
        reward = -self.r_max
        self.no_rew_duration = 0
    end

    --if reward <0 then
    --    print("max reward", self.r_max, "reward", reward)
    --end
    ...
    --end
    --if reward >0 then
    --    self.r_pos_min = math.min(self.r_pos_min, reward)
    --end
