...
local trans = torch.class('dqn.TransitionTable_rs2')
...
function trans:fill_buffer(batch_size)
    ...
    local num_batches = self.bufferSize / batch_size
    local i = 0
    for i=0,(num_batches - 1) do
        local buf_ind_start = i*batch_size + 1
        local mode_batch = 0
        if torch.uniform() < gamma then            
	    for buf_ind=buf_ind_start, buf_ind_start + batch_size - 1 do
                begun = buf_ind - buf_ind_start - batch_size + 1 
		local s, a, r, s2, term = self:sample_one(mode_batch, begun, ind)                
		...
		self.buf_term[buf_ind] = term
            ...
            ind = torch.random(batch_size + 1, self.numEntries-self.recentMemSize)       
            local j = ind
            local counter = 0
            while j<(self.numEntries-self.recentMemSize) do
                if self.r[j] ~= 0 then
                    ind = j
                    mode_batch = 1
                    break
...
function trans:sample_one(mode_batch, begun, ind)
    ...
    while not valid do
     ...
     --[[  original  
        ...
        -- new begin
        if mode_batch == 0 then    
            index = torch.random(2, self.numEntries-self.recentMemSize)
            if self.t[index+self.recentMemSize-1] == 0 then
                valid = true
            end   
        elseif mode_batch == 1 then
            index = ind + begun
            valid = true
        end
        -- new end
...
--    print("final reward: ", self.r[index], buf_ind, mode_batch)
